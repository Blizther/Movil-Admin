...
const ASISTENCIA_ENDPOINT = "https://tornemecsrl.com.bo/assystem/asistencia.php";
...
const korgalDecrypt = (encryptedText, key) => {
  ...
  for (let i = 0; i < encryptedText.length; i++) {
    decrypted += String.fromCharCode(encryptedText.charCodeAt(i) ^ key.charCodeAt(i % key.length));  // XOR entre el carácter y la clave
  }
  return decrypted;
};
...
export default function ScanScreen() {
  ...
  async function handleOpenCamera() {
    ...
    if (!granted) {
      return Alert.alert("Permiso requerido", "Otorga permiso para usar la cámara.");
    }
  ...
  async function handleQRCodeRead(data) {
    ...
    if (!id) {
      return Alert.alert("Código inválido", "No se encontró el ID del trabajador en el QR.");
    }
    ...
    try {
      ...
      const msg = json?.mensaje || (json?.evento ? `Evento: ${json.evento}` : "Evento registrado");
      Alert.alert("Asistencia", msg);
    } catch (e) {
      Alert.alert("Error", (e && e.message) || "No se pudo registrar la asistencia");
    } finally {
      setSending(false);
    }
  }
    ...
    <SafeAreaView style={styles.safe}>
          ...
          <View style={styles.card}>
            <View style={styles.cardHeader}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.cardText}>Apunta al código para registrar asistencia</Text>
              </View>
            </View>
            ...
            >
              ...
              <Text style={styles.btnText}>{sending ? "Procesando..." : "Abrir Cámara"}</Text>
              {sending && <ActivityIndicator color="#fff" style={{ marginLeft: 8 }} />}
            </TouchableOpacity>
      ...
      <Modal visible={modalIsVisible} animationType="slide" transparent={false}>
        ...
        <View style={styles.footerCamera}>
          <TouchableOpacity onPress={() => setModalIsVisible(false)} style={styles.cancelBtn}>
...
const styles = StyleSheet.create({
  ...
  btnText: { color: "white", fontWeight: "700", marginLeft: 8 },
  resultBox: {
    marginTop: 16,
    borderWidth: 1,
    borderColor: "#E2E8F0",
  ...
  resultLabel: { color: "#64748B", fontSize: 12, marginBottom: 6 },
  resultValue: { color: "#0F172A", fontWeight: "700", fontSize: 16 },
  footerCamera: {
    position: "absolute",
    bottom: 24,
    left: 0,
